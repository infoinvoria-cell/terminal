...
11. **Monitoring-Endpoints hart auf 503 gestubbt, auch lokal:** `/api/monitoring/agri-final-status` und `/api/monitoring/wave1-group/[group]` (Route-Code: `return NextResponse.json({error:"unavailable in cloud preview"},{status:503})`, unbedingt, kein Environment-Check) geben **immer** 503 zurück, auch lokal — Code-verifiziert, nicht nur vermutet. Per Live-Netzwerktrace einmal auf `/manager` (Portfolio Lab) beobachtet (4× 503), bei einem zweiten Seitenaufruf jedoch **nicht erneut ausgelöst** — die Requests hängen an einem bestimmten Tab-/UI-Zustand, nicht am Seitenaufruf selbst. Der Code-Fund (immer-503, egal welche Umgebung) ist gesichert; wann genau die UI diese Endpoints aufruft, ist nicht abschließend reproduziert. Nicht selbst geändert, da die Routen in den Monitoring-Datenpfad hineinreichen, der White-Swan-Seiten speist.
