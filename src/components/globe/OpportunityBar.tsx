...
function OppItem({
  ...
  return (
    <button
      ...
      className="ivq-tile flex h-7 w-full items-center justify-between rounded bg-[rgba(18,30,55,0.65)] px-2 text-left text-[10px] text-neutral-100"
    ...
    >
      <span className="mr-1 min-w-0 flex-1 truncate">
        <span className="mr-1 text-neutral-400">{side === "LONG" ? "L" : "S"}</span>
        {item.name}
      </span>
    ...
    <div className="glass-panel ivq-subpanel absolute left-2 right-2 top-2 z-20 p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="rounded border border-neutral-700/70 px-1.5 py-[1px] text-[9px] font-semibold text-neutral-300 transition hover:border-[#9a9a9a]/55 hover:text-[#dce8ff]"
        >
          {open ? "Hide" : "Show"}
        </button>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="space-y-1.5">
              ...
              {!longItems.length && <div className="text-[10px] text-neutral-500">No long setups</div>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              ...
              {!shortItems.length && <div className="text-[10px] text-neutral-500">No short setups</div>}
            </div>
          </div>
        </div>
      ) : null}
