    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", fontFamily: "var(--font-text)", fontWeight: 700, marginRight: 6 }}>#{nr}</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: "#fff", fontFamily: "var(--font-text)" }}>{investor.name}</span>
          {investor.unternehmen && (
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 2, fontFamily: "var(--font-text)" }}>{investor.unternehmen}</div>
          )}
        </div>
        <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 700, fontFamily: "var(--font-text)", background: sc.bg, color: sc.text, flexShrink: 0, marginLeft: 8 }}>
          {investor.status}
        </span>
      </div>
      ...
      {investor.notizen && (
        <div style={{ marginTop: 8, padding: "6px 8px", background: "rgba(255,255,255,0.04)", borderRadius: 6, fontSize: 11, color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-text)", lineHeight: 1.5 }}>
          {investor.notizen}
        </div>
      )}
    </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <span style={{ fontSize: 9.5, color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-text)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}: </span>
      <span style={{ fontSize: 11, color: "rgba(255,255,255,0.65)", fontFamily: "var(--font-text)" }}>{value}</span>
    </div>
  );
}
...
export function MobileInvestorsCRMView() {
  ...
  const inp: React.CSSProperties = { width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e4e4e7", fontSize: 16, fontFamily: "var(--font-text)", padding: "9px 12px", outline: "none" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#090a0c", color: "#e4e4e7" }}>
      <div style={{ padding: "16px 16px 12px", flexShrink: 0, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <h1 style={{ margin: "0 0 2px", fontSize: 16, fontWeight: 800, fontFamily: "var(--font-text)", color: "#fff" }}>Early Access Investoren</h1>
        <p style={{ margin: "0 0 12px", fontSize: 11, color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-text)" }}>{rows.length} Einträge</p>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px 80px" }}>
        {loading ? (
          <p style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 13, paddingTop: 40, fontFamily: "var(--font-text)" }}>Lädt…</p>
        ) : filtered.length === 0 ? (
          <p style={{ textAlign: "center", color: "rgba(255,255,255,0.25)", fontSize: 13, paddingTop: 40, fontFamily: "var(--font-text)" }}>
