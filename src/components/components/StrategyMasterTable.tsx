...
export default function StrategyMasterTable() {
  ...
  // restore product selection from ?product=ws|ci (set by OVERVIEW back arrows)
  useEffect(() => {
    const p = new URLSearchParams(window.location.search).get("product");
    if (p === "ci") { setPortfolio("ci"); setSection("all"); setSortKey("weight"); setSortDir("desc"); }
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%", overflow: "visible", padding: "18px 20px 18px", background: BG, fontFamily: FONT_UI }}>
      ...
      {/* filter bar + overview + live toggle */}
