...
function AurumIcon({ size = 30, spinning = false }: { size?: number; spinning?: boolean }) {
  return (
    <span style={{ position: "relative", display: "inline-flex", width: size, height: size, flexShrink: 0 }}>
      ...
      <img src="/Sentinel.png" alt="Sentinel" width={size * 0.62} height={size * 0.62}
        style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", borderRadius: "50%", objectFit: "cover" }} />
    </span>
  );
}
...
function Bubble({ role, content }: { role: "user" | "assistant"; content: string }) {
  if (role === "user") {
    return (
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div style={{
          maxWidth: "82%", background: `${GOLD}14`, border: `1px solid ${GOLD}30`,
          borderRadius: "14px 14px 4px 14px", padding: "9px 12px",
          fontSize: 13.5, lineHeight: 1.5, color: "#f4f0e6", whiteSpace: "pre-wrap", wordBreak: "break-word",
        }}>
          {content}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", gap: 9, alignItems: "flex-start" }}>
      ...
      <div style={{
        maxWidth: "82%", background: CARD_BG, border: `1px solid ${CARD_BORDER}`,
        borderRadius: "4px 14px 14px 14px", padding: "9px 12px",
        fontSize: 13.5, lineHeight: 1.55, color: "rgba(255,255,255,0.9)",
        whiteSpace: "pre-wrap", wordBreak: "break-word", minHeight: 18,
      }}>
        {content === ""
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 19, fontWeight: 700, color: "#fafafa", fontFamily: "var(--font-montserrat), sans-serif" }}>Sentinel</h1>
        ...
        {!empty && (
          <button onClick={clearChat} aria-label="Chat löschen" style={{
            background: "rgba(255,255,255,0.04)", border: `1px solid ${CARD_BORDER}`,
            borderRadius: 9, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 600,
            padding: "6px 10px", cursor: "pointer",
          }}>Neu</button>
        )}
      </header>

      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12, padding: "4px 16px calc(76px + env(safe-area-inset-bottom, 0px))" }}>
        {empty ? (
          <div style={{ marginTop: 30, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 16 }}>
            <AurumIcon size={64} />
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#fafafa", fontFamily: "var(--font-montserrat), sans-serif" }}>Guten Tag.</div>
              <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.45)", marginTop: 4, maxWidth: 240 }}>Frag mich zu Portfolio, Signalen oder Strategien.</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", marginTop: 4 }}>
              {SUGGESTIONS.map((sug) => (
                <button key={sug} onClick={() => send(sug, { page: "mobile-sentinel" })} style={{
                  textAlign: "left", background: CARD_BG, border: `1px solid ${CARD_BORDER}`,
                  borderRadius: 12, padding: "11px 13px", color: "rgba(255,255,255,0.78)", fontSize: 12.5, cursor: "pointer",
                }}>{sug}</button>
              ))}
            </div>
          </div>
      ...
      <div style={{
        ...
        zIndex: 30, display: "flex", alignItems: "flex-end", gap: 8, padding: "8px 12px",
        background: "rgba(12,13,16,0.96)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)",
        ...
        <textarea
          ...
          style={{
            flex: 1, resize: "none", background: CARD_BG, border: `1px solid ${CARD_BORDER}`,
            borderRadius: 12, padding: "10px 12px", color: "#fafafa", fontSize: 14,
            lineHeight: 1.4, outline: "none", maxHeight: 96, fontFamily: "var(--font-nunito), sans-serif",
          }}
        />
        <button onClick={() => { submit(); if (taRef.current) taRef.current.style.height = "auto"; }}
          disabled={!input.trim()} aria-label="Senden"
          style={{
            ...
            cursor: input.trim() ? "pointer" : "default", transition: "background 140ms ease",
          }}>
