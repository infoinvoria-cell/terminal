...
export function PerformanceYearTable({ trades }: PerformanceYearTableProps) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-auto">
      <table className="w-full min-w-[720px] border-collapse text-left text-[11px] [font-family:var(--font-text),sans-serif]">
        <thead>
          <tr className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
            <th className="sticky left-0 z-10 bg-[#13131A] py-2 pr-3">Year</th>
        ...
        <tbody className="[font-family:var(--font-numbers),sans-serif]">
          {table.rows.map((r) => (
            <tr key={r.year} className="border-t border-white/[0.05]">
              <td className="sticky left-0 z-10 bg-gradient-to-b from-[#1F1F1F] to-[#13131A] py-1.5 pr-3 text-xs font-semibold text-white">
                {r.year}
              </td>
              {r.months.map((v, i) => (
      ...
      <div className="mt-2 flex shrink-0 justify-end border-t border-white/[0.06] pt-2 text-xs">
        <span className="text-zinc-500 [font-family:var(--font-text),sans-serif]">
          Total Return
        </span>
        <span className="ml-3 min-w-[4.5rem] text-right text-sm font-bold text-amber-200/95 [font-family:var(--font-numbers),sans-serif]">
