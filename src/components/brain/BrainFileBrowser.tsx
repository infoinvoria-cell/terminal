...
function renderMd(raw: string): string {
  return raw
    ...
    .replace(/^#{2} (.+)$/gm, `<h2 style="font-size:16px;font-weight:700;color:#fff;margin:24px 0 8px;letter-spacing:-.02em;padding-bottom:6px;border-bottom:1px solid rgba(255,255,255,0.08)">$1</h2>`)
    .replace(/^#{1} (.+)$/gm, `<h1 style="font-size:20px;font-weight:800;color:#fff;margin:0 0 12px;letter-spacing:-.03em;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,0.12)">$1</h1>`)
    // bold / italic
    ...
    <div>
      <div
        ...
        style={{
          ...
          background: isSelected ? "rgba(226,202,122,0.10)" : "transparent",
          color: isSelected ? "#fff" : "rgba(255,255,255,0.70)",
  ...
  return (
    <div style={{ display: "flex", flex: 1, minHeight: 0, minWidth: 0, background: BG, overflow: "hidden" }}>
      ...
      <div style={{
        ...
        display: "flex", flexDirection: "column", background: PANEL, overflow: "hidden",
      }}>
        {/* search */}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          borderBottom: `1px solid ${CBORD}`, background: PANEL, flexShrink: 0,
        }}>
          <span style={{ fontSize: 11, color: MUTED, fontFamily: FONT, cursor: "pointer" }} onClick={() => { setSelected(""); setContent(null); }}>
            Brain
          </span>
          {crumbs.map((c, i) => (
            <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: MUTED, fontSize: 10 }}>›</span>
              <span style={{ fontSize: 11, fontFamily: FONT, color: i === crumbs.length - 1 ? "#fff" : "rgba(255,255,255,0.55)", fontWeight: i === crumbs.length - 1 ? 600 : 400 }}>{c}</span>
            </span>
          ))}
          {fileContent && (
        ...
        <div ref={contentRef} style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
          ...
          {!loading && !selectedPath && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "flex-start" }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#fff", fontFamily: FONT, letterSpacing: "-.03em" }}>Capitalife Brain</div>
          ...
          {!loading && fileContent && (
            <div>
              {isJson ? (
                ...
                <div
                  style={{ fontFamily: FONT, fontSize: 13, lineHeight: 1.75, color: "rgba(255,255,255,0.78)", maxWidth: 760 }}
                  dangerouslySetInnerHTML={{ __html: renderMd(fileContent.text) }}
                />
              ) : (
                <pre style={{ fontFamily: "monospace", fontSize: 11, color: "rgba(255,255,255,0.75)", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                  {fileContent.text}
                </pre>
              )}
            </div>
          ...
          {!loading && selectedPath && !fileContent && !loading && (
            <div>
              {/* Directory listing */}
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: FONT, marginBottom: 16, letterSpacing: "-.02em" }}>
                ...
                {(dirCache[selectedPath] ?? []).map(e => {
                  ...
                  return (
                    <div key={e.name}
                      ...
                      style={{
                        background: "rgba(255,255,255,0.03)", border: `1px solid ${CBORD}`,
