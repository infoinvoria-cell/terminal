...
export const INNO_STRATEGY_CARDS: InnoStrategyCard[] = [
  ...
  },
  {
    ...
    badgeTone: "red",
    rows: [
      { key: "Ziel", value: `Active Alpha 2 — ${CORE_INVEST_MODEL.etfFactorSleeve.length} ETF Factor Sleeve + ${CORE_INVEST_MODEL.managedFuturesOverlay.length} Managed Futures Overlay + Risk Scaling ${CORE_INVEST_MODEL.riskMultiplier}×`, source: "src/data/capitalife/core-invest.config.json" },
      { key: "Status", value: `${CORE_INVEST_MODEL.statusLabel} — keine Live-Execution, kein Broker aktiv`, source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Datenart", value: `${CORE_INVEST_MODEL.validation.etfSeriesReady} ETF-Reihen; ${CORE_INVEST_MODEL.managedFuturesOverlay.length} MF-Roots; ${CORE_INVEST_MODEL.validation.liveReadyComponents} Live-Position (6J short 2 MJY)`, source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Tradingfrequenz", value: "ETF Sleeve: Quartalsrebalancing; Managed Futures: tägliches Trend-Signal L/S", source: "src/data/capitalife/core-invest.config.json" },
      { key: "Haltedauer", value: "ETF Factor Sleeve langfristig (B&H); Managed Futures halten bis Trendsignal-Flip", source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Anzahl Assets", value: `8 ETFs (140% gross, -40% BIL) + 12 Futures-Roots (ES, NQ, 6E, 6J, 6B, 6S, GC, HG, CL, NG, ZC, ZS)`, source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Risk Scaling", value: `${CORE_INVEST_MODEL.riskMultiplier}× Multiplier · ${CORE_INVEST_MODEL.longExposureCap}× Long Cap · ${CORE_INVEST_MODEL.grossLongExposure * 100}% Brutto-Long via BIL-Cash-Finanzierung`, source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Track-Record-Status", value: "Kein Live-Track-Record — Final Ablation: Sharpe 0.663, CAGR +14.66%, MaxDD −28.33%, Calmar 0.517 (Research/Pre-Fund)", source: "Brain/05_Portfolios/Core_Invest/Core Invest Strategy.md" },
      { key: "ETF Gewichte", value: "SPY 56%, QQQ 28%, VLUE 16%, RSP 8.4%, QUAL 8.4%, MTUM 8.4%, USMV 8.4%, IWM 6.4%, BIL -40%", source: "src/lib/components/ws-strategy-data.ts" },
      { key: "Instrumententypen", value: "ETFs + Micro-Futures (MES, MNQ, M6E, MJY, M6B, MSF, 1OZ, MHG, MCL, MNG, MZC, MZS)", source: "src/lib/core-invest/core-invest-model.ts" },
      { key: "Offene Pruefungen", value: "Live-Signalengine, Futures-Rolls, IBKR-Mapping, Walk-Forward, Parity-Check, Broker-Freigabe", source: "Brain/05_Portfolios/Core_Invest/00 Core Invest MOC.md" },
    ],
  },
];
