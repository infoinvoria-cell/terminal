...
const ANOMALY: StrategyRow[] = [
  {
    id: "gc1_friday", ticker: "GC1!", label: "Gold Futures — Friday Intraday v3", group: "Anomaly",
    engine: "KEEP · Fri 00:00 UTC close -> last Fri bar close · ATR14 SL1.0 · no filter · weight=0% (GLD overlap)", pillar: "anomaly", weight: 0,
    sharpeOos: null, cagr: null, maxDd: null, calmar: 0.23, pf: 1.16, trades: 584, wfOos: "8/13",
    status: "watch", dataFile: "anomaly/gc1_friday_long.json", exchange: "COMEX",
    isNotes: "ROLL-INDEPENDENCE TEST 2026-08-10 · STRATEGY: KEEP · ALLOCATION: 0% (GLD corr r=0.62 dilutes Calmar) · Roll removed: 132/1150 trades (11.5%) removed as CME roll-window artifacts · Roll PF=2.65 vs non-roll PF=1.21 (artifact confirmed) · NON-ROLL WFO OOS (pre-2021, 13 folds): PF=1.1619, exp_R=+0.148R, 8/13 folds positive, Calmar-R=0.23, MaxDD-R=28.9R · Temporal neighborhood non-roll: all 3 entry variants PF>1.32 · DST: edge in both US STANDARD and DAYLIGHT periods · KHV 2021-2026 non-roll (NOT pristine OOS): PF=1.08 (weaker, positive) · Cost GC 2x stress PF=1.21 PASS; MGC 2x PF=1.13 PASS · Individual GC contracts unavailable - continuous-series artifact cannot be fully eliminated · Portfolio gate: FAIL - adding GC to GLD+YM degrades Calmar (high GLD-GC annual corr r=0.62; GLD already captures gold intraday momentum via different window) · Forward tracking active; allocation requires uncorrelated return evidence · Locked: Fri 00:00 UTC close / last Fri bar close / ATR=14 SL=1.0 / no filter · script: run_white_swan_gold_friday_roll_test.py · report: white_swan_gold_friday_roll_test.json",
  },
  {
