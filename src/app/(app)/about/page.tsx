  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden px-7 pb-4 pt-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <BookOpen size={14} className="shrink-0 text-[color:var(--dash-accent)]" />
        <span className="text-[13px] font-bold text-white" style={{ fontFamily: N }}>Bibel</span>
        <span className="ml-1 text-[10px] text-[color:var(--dash-muted)]" style={{ fontFamily: M }}>Intern · kein Angebot · nicht geprüft</span>
      </div>

      {/* ROW 1 — 2 Strategien */}
      <div className="grid min-h-0 flex-1 grid-cols-2 gap-3">
        {ABOUT_STRATEGIES.map((s) => (
          <div key={s.id} className="flex min-h-0 flex-col overflow-hidden rounded-[18px] border border-white/[0.07] bg-gradient-to-b from-[#1e1f22] to-[#151618] p-5 shadow-[0_12px_32px_-12px_rgba(0,0,0,0.6)]">
            {/* Title */}
            <div className="mb-3 flex items-start justify-between">
              <div>
                <p className="mb-0.5 text-[10px] font-semibold uppercase tracking-widest text-[color:var(--dash-muted)]" style={{ fontFamily: M }}>{s.number} · {s.type}</p>
                <h2 className="text-[22px] font-bold leading-tight text-white" style={{ fontFamily: N, letterSpacing: "-0.02em" }}>{s.name}</h2>
              </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[1fr_256px] gap-3">
        {/* Tabelle */}
        <div className="flex min-h-0 flex-col overflow-hidden rounded-[18px] border border-white/[0.07] bg-gradient-to-b from-[#1e1f22] to-[#151618] shadow-[0_12px_32px_-12px_rgba(0,0,0,0.6)]">
          <div className="flex shrink-0 items-center gap-2 border-b border-white/[0.05] px-5 py-2.5">
            <BarChart2 size={12} className="text-[color:var(--dash-accent)]" />
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[color:var(--dash-accent)]" style={{ fontFamily: M }}>Vergleich · Anlageklassen</p>
          </div>
        ...
        <div className="flex min-h-0 flex-col gap-3">
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[18px] border border-white/[0.07] bg-gradient-to-b from-[#1e1f22] to-[#151618] p-4 shadow-[0_12px_32px_-12px_rgba(0,0,0,0.6)]">
            <SectionHead icon={<Wallet size={12} />} label="Für Investoren" />
            <div className="mt-2.5 space-y-2">
              {ABOUT_INVESTOR.map(({ key, value }) => <KV key={key} k={key} v={value} />)}
            </div>
          </div>
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[18px] border border-white/[0.07] bg-gradient-to-b from-[#1e1f22] to-[#151618] p-4 shadow-[0_12px_32px_-12px_rgba(0,0,0,0.6)]">
            <SectionHead icon={<Shield size={12} />} label="Risikoprofil" />
            <div className="mt-2.5 space-y-2">
...
function BigStat({ label, value, color }: { label: string; value: string; color: "gold"|"red"|"white" }) {
  ...
  return (
    <div className="rounded-[10px] border border-white/[0.05] bg-white/[0.03] px-3 py-2.5">
      <p className="mb-1 text-[9px] text-[color:var(--dash-muted)]" style={{ fontFamily: M }}>{label}</p>
      <p className={`text-[18px] font-bold leading-none ${c}`} style={{ fontFamily: N }}>{value}</p>
    </div>
  );
}
...
function Row({ icon, k, v }: { icon: React.ReactNode; k: string; v: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] text-[color:var(--dash-muted)]" style={{ fontFamily: M }}>{k}</span>
      <span className="ml-auto text-right text-[10px] font-medium text-white" style={{ fontFamily: M }}>{v}</span>
    </div>
  );
}
...
function KV({ k, v }: { k: string; v: string }) {
  return (
    <div className="flex items-baseline justify-between gap-2">
      ...
      <span className="text-right text-[10px] font-medium text-white" style={{ fontFamily: M }}>{v}</span>
    </div>
  );
}
...
function TRow({ name, tag, cagr, dd, sharpe, calmar, horizon, corrSpy, accent }: {
  ...
  return (
    <tr className={accent ? "bg-[color:var(--dash-accent)]/[0.03]" : ""}>
      ...
      <td className="px-4 py-2 text-[11px] font-semibold text-[color:var(--dash-accent)]" style={{ fontFamily: N }}>{cagr}</td>
