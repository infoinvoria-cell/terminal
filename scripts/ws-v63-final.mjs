...
// ZW: v6.3 FIX — gross-only scale × 0.1, then subtract actual MZW IBKR cost independently
// v6.2 bug: applied × 0.2 to netEUR (scaled both gross AND the already-embedded ZW cost)
// zw-backtest-results.json has: {grossUSD, netUSD, netEUR} where grossUSD = ZW×50 price move × mult50
const zw_trades = zwRes.bestCandidate.trades.map(t => {
  const bookDate = t.exitDate ?? t.entryDate;
  return {
    date: bookDate,
    // MZW gross = ZW×50 gross × 0.1 (MZW is 1/10 of ZW full; mult50 already applied in source)
    // MZW cost = actual IBKR MZW all-in RT (NOT scaled from ZW cost)
    netEUR: +(((t.grossUSD * 0.1) - MZAG_COST_RT_USD) / fxRate(bookDate)).toFixed(4),
  };
});
...
const gc1_trades = allTrades
  .filter(t => t.strategyId === 'gc1_seasonal')
  .map(t => ({
    date: t.exitDate ?? t.entryDate,
    // MGC gross = 1-oz gross × 10 oz per MGC contract
    // MGC cost = actual IBKR all-in RT (NOT legacy t.costRt=1.9 which is wrong basis)
    netEUR: +(((t.grossPnl * 10) - MGC_COST_RT_USD) / fxRate(t.exitDate ?? t.entryDate)).toFixed(4),
  }));
...
const UNIVERSE = [
  ...
  // ── Non-core tradable ──────────────────────────────────────────────────────
  { id: 'cc_seasonal',  label: 'CC Seasonal',              inst: 'CC',   margin: 3150, costRt: 4.72, core: false, blocked: false, maxCt: 2,  trades: buildFromAllTrades('cc_seasonal') },
  { id: 'spy_mes',      label: 'SPY/MES S&P Seasonal',    inst: 'MES',  margin: 1390, costRt: 1.22, core: false, blocked: false, maxCt: 8,  trades: buildFromAllTrades('spy_seasonal') },
  { id: 'cl1_seasonal', label: 'CL Seasonal',              inst: 'MCL',  margin: 1390, costRt: 1.52, core: false, blocked: false, maxCt: 8,  trades: buildFromAllTrades('cl1_seasonal') },
  { id: 'hg1_seasonal', label: 'HG Seasonal',              inst: 'MHG',  margin: 2500, costRt: 1.92, core: false, blocked: false, maxCt: 4,  trades: buildFromAllTrades('hg1_seasonal') },
  { id: 'zm1_seasonal', label: 'ZM Seasonal',              inst: 'MZM',  margin: 1250, costRt: MZAG_COST_RT_USD, core: false, blocked: false, maxCt: 8,  trades: buildFromAllTrades('zm1_seasonal') },
  { id: 'ym1_tat',      label: 'YM1 TAT',                  inst: 'MYM',  margin: 765,  costRt: 1.22, core: false, blocked: false, maxCt: 8,  trades: buildFromAllTrades('ym1_tat') },
  ...
  { id: 'sb_seasonal',  label: 'SB Seasonal',              inst: 'SB',   margin: 1070, costRt: 4.72, core: false, blocked: false, maxCt: 4,  trades: buildFromAllTrades('sb_seasonal') },
  { id: 'zc_seasonal',  label: 'ZC Seasonal',              inst: 'MZC',  margin: 520,  costRt: MZAG_COST_RT_USD, core: false, blocked: false, maxCt: 8,  trades: buildFromAllTrades('zc_seasonal') },
  { id: 'zs_seasonal',  label: 'ZS Seasonal',              inst: 'MZS',  margin: 1220, costRt: MZAG_COST_RT_USD, core: false, blocked: false, maxCt: 4,  trades: buildFromAllTrades('zs_seasonal') },
  // ── Blocked ───────────────────────────────────────────────────────────────
