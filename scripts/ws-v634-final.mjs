...
// ═══════════════════════════════════════════════════════════════════════════
console.log('\n\n=== DAX CONCENTRATION SANITY CHECK (all 10 tiers) ===');

const fdaxDailyCsv = fs.readFileSync('data/historical/indices/EUREX_FDAX1_D.csv', 'utf8').trim().split('\n').slice(1);
const fdaxDailyMoves = [];
{
  let prevClose = null;
  for (const line of fdaxDailyCsv) {
    const [date, , , , closeStr] = line.split(',');
    const close = parseFloat(closeStr);
    if (prevClose !== null) {
      const chg = Math.abs(close - prevClose);
      const pct = chg / prevClose * 100;
      if (pct < 15) fdaxDailyMoves.push({ date, chg, pct }); // filter roll artifacts
    }
    prevClose = close;
  }
}
fdaxDailyMoves.sort((a, b) => b.chg - a.chg);
const fdaxWorstDay = fdaxDailyMoves[0];
console.log(`FDAX_WORST_DAY_REAL: ${fdaxWorstDay.chg.toFixed(1)}pt move on ${fdaxWorstDay.date} (${fdaxWorstDay.pct.toFixed(2)}%)`);

const FDXS_MULT_REF = 1; // EUR/point for FDXS micro DAX
const daxConcentration = tierResults.map(tier => {
  const cts = tier.recommended.contracts;
  const dax1hCt = cts.dax_1h ?? 0;
  const dax2hCt = cts.dax_2h ?? 0;
  const maxSimultaneousDax = dax1hCt + dax2hCt; // worst case: both sleeves hold a position the same day
  const maxDaxDailyLossEUR = Math.round(fdaxWorstDay.chg * FDXS_MULT_REF * maxSimultaneousDax);
  const totalMargin = tier.recommended.feasibility?.totalMargin ?? 0;
  const freeCash = tier.capital - totalMargin;
  const minExcessLiquidity = freeCash - maxDaxDailyLossEUR;
  const survivalPass = minExcessLiquidity > 0;

  const portfolioMaxDDEUR = tier.recommended.fullKPIs?.MaxDDEUR ?? 1;
  const daxShareOfMaxDD = portfolioMaxDDEUR > 0 ? +(Math.min(maxDaxDailyLossEUR / portfolioMaxDDEUR, 1) * 100).toFixed(1) : 0;
  // Rough vol share: DAX margin as % of total margin (proxy for capital-at-risk share, since true per-sleeve
  // daily P&L variance decomposition would require a full covariance matrix — out of scope for this gate)
  const daxMargin = dax1hCt * 880 + dax2hCt * 880;
  const daxShareOfVol = totalMargin > 0 ? +(daxMargin / totalMargin * 100).toFixed(1) : 0;

  return {
    capital: tier.capital, dax1hCt, dax2hCt, maxSimultaneousDax,
    maxDaxDailyLossEUR, daxShareOfPortfolioVolPct: daxShareOfVol, daxShareOfPortfolioMaxDDPct: daxShareOfMaxDD,
    minExcessLiquidityDuringWorstDaxDayEUR: Math.round(minExcessLiquidity),
    survivalPass: survivalPass ? 'PASS' : 'FAIL',
  };
});
daxConcentration.forEach(d => {
  console.log(`  €${(d.capital/1000).toFixed(0)}k: DAX1H×${d.dax1hCt} DAX2H×${d.dax2hCt} maxSimultaneous=${d.maxSimultaneousDax} worstDayLoss=€${d.maxDaxDailyLossEUR} volShare=${d.daxShareOfPortfolioVolPct}% maxDDShare=${d.daxShareOfPortfolioMaxDDPct}% minExcessLiquidity=€${d.minExcessLiquidityDuringWorstDaxDayEUR} SURVIVAL=${d.survivalPass}`);
});
...
fs.writeFileSync('workspace/output/white-swan/v6.3/dax-concentration-survival.json', JSON.stringify({
  fdaxWorstDayReal: fdaxWorstDay, allTiersPass: allDaxSurvivalPass, tiers: daxConcentration,
}, null, 2));
