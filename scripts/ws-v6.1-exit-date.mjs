/**
 * ws-v6.1-exit-date.mjs
 * WHITE SWAN — v6.1 EXIT-DATE P&L CORRECTION
 *
 * v6.1 forensic fixes over v6:
 * 1. P&L assigned to exitDate (not entryDate) for all multi-day strategies.
 *    For gld_atr2080_trades.json (only has `date`): `date` is treated as exit date (unchanged).
 *    For eurusd_30m (intraday): exitDate == entryDate, no behavioral change.
 * 2. bestSmallAccount selection uses isCAGR (IS-only) instead of oos2019CAGR to
 *    eliminate OOS19 double-use (optimization + selection on same metric).
 * 3. Output directory changed to v6.1 / serkan/v6.1.
 * 4. Disclosure: MaxDD is based on exit-date realized P&L. Intraday MTM during
 *    open positions is not modeled. True MTM drawdown may exceed reported figures.
 *
 * Unchanged from v6:
 * - Hard constraint: Core 5 must have >= 1 contract at every tier.
 * - Sharpe: all Mon-Fri trading days 2008-2025, 0-fill on non-trade days.
 * - Scoring: OOS CAGR-biased (weight 5.0).
 * - Optimizer candidates: MAX_RETURN(0.82), BALANCED(0.75), MAX_SHARPE(0.72), MAX_CALMAR(0.68).
 */
import fs from 'fs';
...
function buildFromAllTrades(stratId, filterFn) {
  // v6.1 FIX: use exitDate (trade is realized on close, not open).
  // Falls back to entryDate for intraday strategies where exitDate == entryDate.
  return allTrades.filter(t => t.strategyId === stratId && (!filterFn || filterFn(t))).map(t => ({
    date: t.exitDate ?? t.entryDate, netEUR: +tradeNetEUR(t).toFixed(4),
  }));
}
...
// ─── Strategy universe ─────────────────────────────────────────────────────────
// eurusd_30m is genuinely intraday (entryDate == exitDate in all samples).
// buildFromAllTrades now uses exitDate which equals entryDate — no behavioral change.
// Filter still uses entryDate (day-of-week filter on the trade open date is semantically correct).
const eu_trades = allTrades.filter(t => {
  ...
  date: t.exitDate ?? t.entryDate, netEUR: +(tradeNetEUR(t) * 0.1).toFixed(4),
}));

// v6.1 FIX: ZW uses exitDate (zw-backtest-results.json has both entryDate and exitDate).
...
// ─── Summary stats ────────────────────────────────────────────────────────────
// v6.1 FIX: Use isCAGR (IS-only) for bestSmallAccount selection.
// Eliminates OOS19 double-use: oos2019CAGR was used in both scorePortfolio() and here,
// making the "best" capital selection circular (optimized for OOS19, then ranked by OOS19).
// isCAGR is an independent measure not in the scoring objective.
const bestSmall = tierResults.filter(r => r.capital <= 25000 && r.recommended.feasibility.feasible)
  .sort((a, b) => (b.recommended.kpis?.isCAGR ?? 0) - (a.recommended.kpis?.isCAGR ?? 0))[0];
...
// ─── Serkan CSVs ───────────────────────────────────────────────────────────────
fs.mkdirSync('workspace/output/white-swan/v6.1', { recursive: true });
fs.mkdirSync('workspace/output/white-swan/serkan/v6.1', { recursive: true });
...
for (const cap of SERKAN_CAPS) {
  ...
  fs.writeFileSync(`workspace/output/white-swan/serkan/v6.1/${fname}`, csv);
...
fs.writeFileSync('workspace/output/white-swan/serkan/v6.1/white_swan_final_daily_returns.csv', finalCsv);
...
const summary = {
  ...
  status: 'CORE_PRESERVING_EXIT_DATE_CORRECTED',
  description: 'v6.1: P&L booked on exitDate (not entryDate) for all strategies. bestSmallAccount uses isCAGR (no OOS19 double-use). MaxDD = exit-date realized only — intraday MTM not modeled.',
  filterImprovements: {
  ...
  navMethodNote: 'P&L assigned to trade exit date (v6.1 correction from entry-date booking). MaxDD reflects exit-date realized P&L only. Intraday MTM during open multi-day positions is not modeled. True drawdown in stressed markets may exceed reported MaxDD.',
  components: recCapData?.components ?? [],
  capitalComparison,
  portfolioKPIs: recKPIs,
  capitalLadder: ladder,
  bestSmallAccount: bestSmall ? { capital: bestSmall.capital, isCAGR: bestSmall.recommended.kpis?.isCAGR, selectionCriterion: 'isCAGR (v6.1 — independent of OOS19 scoring)' } : null,
  ...
  serkan: {
    path: 'workspace/output/white-swan/serkan/v6.1/',
    finalRows: finalRows.length, dateRange: ['2008-01-02', '2025-12-31'],
    perCapitalAudit: serkanAudit,
  },
};

fs.writeFileSync('workspace/output/white-swan/v6.1/portfolio-summary.json', JSON.stringify(summary, null, 2));
fs.writeFileSync('public/data/white-swan/final/portfolio-summary.json', JSON.stringify(summary, null, 2));
fs.writeFileSync('workspace/output/white-swan/v6.1/equity-series.json', JSON.stringify({ series: equitySeries, yearlyReturns }));
...
console.log(`BEST SMALL ACCOUNT: €${bestSmall?.capital ?? '?'} — isCAGR: ${bestSmall?.recommended?.kpis?.isCAGR ?? '?'}% (v6.1: selected by IS CAGR, not OOS19)`);
